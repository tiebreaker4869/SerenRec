...
class SASRecDataset(Dataset):
    def __init__(self, user_seqs, num_items, max_len, is_training=True, mask_prob=0.2, neg_sample=1):
    ...
    def __getitem__(self, index):
        ...
        if self.is_training:
            ...
            # 随机选择一个正样本位置
            pos_idx = np.random.randint(1, len(seq))
            pos_item = seq[pos_idx]
            ...
            attention_mask = [0] * pad_len + [1] * len(seq[:pos_idx])
        ...
        # 处理验证或测试数据 (验证集和测试集是元组 (input_seq, target_item))
        else:
            # 获取输入序列和目标物品
            ...
            # 填充序列到固定长度
            pad_len = self.max_len - len(input_seq)
            input_seq_padded = [0] * pad_len + list(input_seq)  # 确保转换为列表
            
            # 创建注意力掩码
            attention_mask = [0] * pad_len + [1] * len(input_seq)
            
            return {
                'input_seq': torch.LongTensor(input_seq_padded),
                'attention_mask': torch.LongTensor(attention_mask),
...
class SASRec(nn.Module):
    ...
    def forward(self, input_seq, attention_mask=None):
        ...
        # 结合物品嵌入和位置编码
        seqs = item_emb + pos_emb
        seqs = self.dropout(self.layer_norm(seqs))
        
        # 转化为Transformer期望的注意力掩码格式
        if attention_mask is not None:
            # 这里attention_mask中的0表示padding位置，1表示有效位置
            # 需要将其转换为transformer要求的格式：False表示保留，True表示掩盖
        ...
        for layer in self.transformer_layers:
            seqs = layer(seqs, src_key_padding_mask=extended_attention_mask)
        
        return seqs
    
    def predict(self, seqs, item_indices=None):
        """
        计算序列对目标物品的预测分数
        
        Args:
            seqs: 序列表示，形状为[batch_size, seq_len, hidden_size]
            item_indices: 目标物品索引，形状为[batch_size, 1]，如果为None则计算所有物品
            
        Returns:
            prediction: 预测分数
        """
        # 获取序列的最后隐藏状态（最后一个非pad位置）
        last_hidden = seqs[:, -1, :]
        ...
        if item_indices is not None:
            ...
            # 计算内积作为预测分数
            prediction = torch.sum(last_hidden * item_emb, dim=-1, keepdim=True)
        else:
            # 计算与所有物品的分数
            all_items = self.item_embedding.weight  # [num_items, hidden_size]
            prediction = torch.matmul(last_hidden, all_items.transpose(0, 1))  # [batch_size, num_items]
...
def train(model, train_loader, optimizer, device, args):
    ...
    for batch in progress_bar:
        ...
        # 前向传播
        optimizer.zero_grad()
        seqs = model(input_seq, attention_mask)
        
        # 计算正样本分数
        pos_score = model.predict(seqs, pos_item)
        
        # 计算多个负样本分数
        neg_scores = []
        for i in range(neg_items.size(1)):
            neg_item = neg_items[:, i].unsqueeze(1)
            neg_score = model.predict(seqs, neg_item)
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc=f"Evaluating {name}"):
            ...
            for i, seq in enumerate(input_seq):
                for item in seq:
                    if item != 0:  # 排除padding
                        predictions[i, item] = -np.inf
            
            # 获取每个用户的top-k推荐
            max_k = max(k_list)
            _, topk_indices = torch.topk(predictions, k=max_k)
            topk_indices = topk_indices.cpu().numpy()
            
            # 更新指标
            valid_user_count += input_seq.size(0)
...
def main(args):
    ...
    if args.save_model:
        with open(os.path.join(args.output_dir, 'data_stats.txt'), 'w') as f:
            ...
            # f.write(f"平均序列长度: {avg_seq_len:.2f}, 最大序列长度: {max_seq_len}\n")
...
def parse_args():
    ...
    parser.add_argument('--mask_prob', type=float, default=0.0, help='掩码概率')
    parser.add_argument('--neg_sample', type=int, default=1, help='负采样数')
